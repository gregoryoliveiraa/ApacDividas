                ...
                <div class="ibox ">
                    ...
                    <?php
                        ...
                        if($idColegioSessao > 0){
                            $whereClause = "WHERE C. ID_COLEGIO = {$idColegioSessao}";
                        } 

                        $sql = "SELECT R.NOME AS NOMER, * FROM RESPONSAVEL AS R
                        LEFT JOIN ALUNO AS A ON A.ID_RESPONSAVEL = R.ID_RESPONSAVEL
                        LEFT JOIN COLEGIO AS C ON A.ID_COLEGIO = C.ID_COLEGIO {$whereClause}";
                        $stmt = sqlsrv_query($conn, $sql);
                        if ($stmt === false) {
                            die(print_r(sqlsrv_errors(), true));
                        }
                        ...
                        while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
                            echo "<tr class='gradeA'>
                                        ...
                                        if(!$acessoUsuario){
                                            ...
                                            <a href='responsavel.php?id=".$row['ID_RESPONSAVEL']."' title='Editar'><button class='btn btn-success btn-circle' type='button'><i class='fa fa-edit'></i></button></a>&ensp;
                                            <a href='javascript:deletar(".$row['ID_RESPONSAVEL'].");' title='Deletar'><button class='btn btn-danger btn-circle' type='button'><i class='fa fa-trash'></i></button></a>
                                            </td>";
                                        }
                                    echo "</tr>";
                                    }
